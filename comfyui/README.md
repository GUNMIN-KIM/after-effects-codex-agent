...
사용자 피드백("AI 영역과 원본의 화질이 다르고, 서로 맞지 않고, 경계가 티 난다")을 반영해 구조를 바꿨다.

| 문제 | 원인 | 수정 |
|---|---|---|
| 서로 매치 안 됨 | Stage 1이 가운데까지 새로 생성 → 바깥은 AI가 그린 가운데에 맞춰짐 | **Stage 1부터 원본 영역을 latent로 고정**. 원본(절반 해상도)을 인코드하고 `LTXVSetAudioVideoMaskByTime`의 `spatial_mask`로 가운데 0 · 바깥 1. 바깥이 처음부터 실제 원본을 보며 생성된다 (+ 공식 In/Outpaint IC-LoRA 가이드) |
| 화질 다름 | 긴 변 1920으로 생성 후 캔버스로 확대 · Stage 2의 가운데가 흐린 확대본 | **생성 해상도 = 캔버스 원본 크기**(④ 기본 4096 = 제한 없음). Stage 2는 **풀해상도 원본을 가운데에 넣고 고정**한 뒤 바깥을 sigma 0.85부터 정제 |
| 경계 티 | 내용이 다른 두 영상을 딱 붙임 | 원본 안쪽 **⑨ 24px 띠**에서만 Laplacian 블렌드. 가운데가 고정돼 있어 띠 안의 생성본은 원본과 거의 같다 |

그 밖의 처리는 다음과 같다.
- 가이드 프레임은 공간 마스크 범위(본 영상 길이)에서 제외돼 계속 고정된다. IC-LoRA 축소 비율과 무관하게 동작하며, 모의 객체로 노드를 직접 실행해 확인했다.
- 생성 영역 전체 색을 원본에 맞춘다(⑧).
- 원본 fps를 유지하고, 원본 오디오는 모델에 고정 입력하고 출력에는 원본 파형을 그대로 쓴다.
- 프레임 수가 8n+1이 아니면 마지막 프레임을 반복해 채운 뒤 원래 길이로 자른다.

VRAM이 부족해 ④를 낮추면 바깥 영역만 확대되므로 화질 차이가 다시 생길 수 있다.
