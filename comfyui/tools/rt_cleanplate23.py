...
from runtest import *
p=json.load(open('/opt/cf/out_cleanplate23.api.json'))
dec=find(p,'VAEDecodeTiled')[0]
fM=find(p,'ImageFromBatch','8n+1')[0]
wf=find(p,'ComfyMathExpression','생성 가로')[0]; hf=find(p,'ComfyMathExpression','생성 세로')[0]
...
print("OK" if run(p,'cleanplate23-stub') else "FAIL")
